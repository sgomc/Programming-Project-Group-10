class MainMenu extends Window { //<>// //<>// //<>//
  ...
  void display() {
    ...
    // Draw the scrollbar
    if (!stateAirports.isEmpty() && stateAirports.size() > airportsPerPage) {
      int scrollBarX = w - SCROLLBAR_WIDTH - 10;  // Positioned at the right edge
      int scrollTrackY = tableBodyY;

      fill(180);
      stroke(0);
      rect(scrollBarX, scrollTrackY, SCROLLBAR_WIDTH, tableBodyHeight - 55);
      noStroke();

      fill(100);
      rect(scrollBarX, thumbY, SCROLLBAR_WIDTH, thumbHeight - 55);
    }
  ...
  ArrayList<Airport> getAirportsForCurrentState() {
    ...
    if (currentState != null) {
      String stateAbbreviation = getStateAbbreviation(getStateName(currentState));
  ...
  void mousePressed(int mx, int my) {
    ...
    if (sortCitiesCheckBox.checkClick(relativeX, relativeY)) {
      // Toggle the checkbox state explicitly
      //sortCitiesCheckBox.isChecked = !sortCitiesCheckBox.isChecked;
